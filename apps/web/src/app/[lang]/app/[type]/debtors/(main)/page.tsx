...
export default function Page({
  ...
  const fetchLink = getBaseLink(`/api/crm/merchants`);
  const [roles, setRoles] = useState<any>();
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const router = useRouter();
  const [formData, setFormData] = useState<TableData>(
    dataDebtors.debtors.pages.debtors,
  );
  const { resources } = useLocale();
  const languageData = getResourceDataClient(resources, params.lang);
  const detailedFilters =
    dataDebtors.debtors.pages.debtors.detailedFilters || [];
  ...
  if (createFormSchema) {
    action = [
      {
        cta: languageData["Debtor.New"],
        type: "NewPage",
        href: `/app/admin/debtors/new`,
      },
      {
        cta: `Export CSV`,
        callback: () => {
  ...
  columnsData.data.actionList?.push({
    ...
    type: "Action",
    callback: (row) => {
      router.push(getBaseLink(`app/admin/debtors/${row.id}`));
    },
  });
  return (
    <Dashboard
